...
export default function Footer() {
  ...
  return (
    <footer className="site-footer">
      <Reveal variant="fade-up" duration={480}>
        <div className="footer-main">
            ...
            <ul className="footer-contact-list">
              {brand.contactItems.map((item) => {
                ...
                return (
                  <li key={item.id}>
                    {item.href ? (
                      <a href={item.href} className="footer-contact-link transition-colors duration-200 hover:text-white">
                        {content}
                      </a>
                    ) : (
            ...
            <nav aria-label={navigation.ariaLabel}>
              <ul className="footer-nav-list">
                {navigation.links.map((link) => (
                  <li key={link.href}>
                    <a href={link.href} className="footer-nav-link group transition-colors duration-200 hover:text-white">
                      <span>{link.label}</span>
                      <span className="footer-nav-arrow motion-arrow-icon" aria-hidden="true">
                        ›
                      </span>
                    </a>
                  </li>
            ...
            <ul className="footer-services-list">
              {services.links.map((service) => (
                <li key={service.label}>
                  <a href={service.href} className="footer-service-link transition-colors duration-200 hover:text-white">
                    <FooterIcon
                      name={service.icon}
                      className="footer-service-icon transition-transform duration-200 hover:scale-110"
                    />
              ...
              {social.links.map((link) => {
                ...
                return (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      className="footer-social-link transition-transform duration-200 hover:-translate-y-0.5"
          ...
          <section className="footer-column footer-newsletter-column">
            ...
            >
              ...
              <button type="submit" className="footer-newsletter-submit transition-transform duration-200 hover:-translate-y-0.5">
                <span>{newsletter.form.buttonLabel}</span>
                <Send aria-hidden="true" className="motion-arrow-icon" />
              </button>
            </form>
          </section>
        </div>
          ...
          <nav className="footer-legal" aria-label={bottomBar.legalAriaLabel}>
            {bottomBar.legalLinks.map((link, index) => (
              <span key={link.href} className="footer-legal-item">
                ...
                <a href={link.href} className="footer-legal-link transition-colors duration-200 hover:text-white">
                  {link.label}
                </a>
              </span>
            ))}
